...
- A *system* has components and purpose. The interaction between the components (coupling) are needed for the components to achieve the system's goal. On a smaller scale, a system's component is almost always a subsystem in its own right.
...
- A *module* is a type of component designed to improve the flexibility of a system, allowing it to evolve to support future goals.
  - A *software module* is a boundary encompassing a well-defined functionality, which it exposes (through its public interface) for use by other parts of the system. It can represent any logical or physical boundary within a software system: a service, a namespace, an object, a function, etc.
...
- A system's design should not be evaluated by examining individual modules in isolation, but the relationships between them: how they are coupled. Different ways of coupling components share different types and amounts of knowledge. Some will increase complexity, while other will contribute to modularity.
- Coupling manifests across three dimensions: strength, space (distance), and time (volatility).
  - The higher the integration strength (the more knowledge shared across their boundaries), the higher the likelihood that modules will need to change in unison. When such cascading changes occur, distance signifies the effort required to implement them. Volatility represents how frequently these changed are anticipated to happen.
- *Integration strength* models the type and extent of knowledge shared by coupled components. It differentiates between four levels of coupling strength:
  ...
  - Functional coupling: components implement closely related business functionalities. A change in the business requirements are highly likely to affect all coupled components.
  - Model coupling: components using a shared model of the business domain. The evolution of the model requires changes in all coupled models.
...
- *Distance* refers to the space knowledge "travels" across coupled components. It affects the coordination and communication efforts needed to implement a change affecting the coupled components. Distance is influenced by several factors:
  - The physical location of the code: for example, it's easier to change coupled methods in a class, than coupled methods across distributed systems. On the other hand, collocating otherwise unrelated functionalities couples their lifecycles, introducing collateral changes that wouldn't be necessary if they were separate.
  - Runtime coupling: for example, not having to introduce changes simultaneously across systems reduces coordination efforts.
  - Organization structure: the greater the /ownership/ distance (whether components are mantained by the same person, team, department, or organization), the higher the coordination effort. Given Conway's law, over time, the ownership distances will influence the encapsulation distance of the system's modules.
- *Volatility* is a component's expected rate of change.
  ...
  - Investing in modularization (adjusting integration strength and distance to reduce maintenance cost) won't be as beneficial for components that are not expected to change frequently.
- *Balance* refers to the optional state of the three dimensions of coupling, in which the design increases the modularity of the system's volatile components. An unbalanced combination of these dimensions results in increased cognitive load and, consequently, increased complexity.
  - Not all of a large system will be well-designed, some components are more important business-wise than others.
...
#+begin_src vhdl
-- there's global complexity when much knowledge
...
-- there's modularity when there's no local nor global complexity
