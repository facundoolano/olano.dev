...
- [[https://jorge.olano.dev/blog/on-ai-assistance/][On AI assistance]]: I start to occasionally reach to ChatGPT during an open-source project development, especially for chores like "I wish there was a lib for this", "give me the Makefile incantation for this task". I see potential; the results are hit and miss. I make plans to better integrate the tech to my workflow.
- [[augmentation-replacement][Augmentation / Replacement]]: I incorporated LLMs to my workflow through Emacs's [[https://github.com/karthink/gptel][gptel]], and see myself using it daily, as a rubber duck and Google's L1 Cache. I recognize the human augmentation potential of AI---not so much for what LLMs are today, but for the taste we get of what a better AI could be---, but trying to use them as human replacements is short-sighted and counter-productive.
- [[agentic-coding-experience][Quick notes on a brief agentic coding experience]]: I naively try to use Claude Code to implement some planned features of a working web application, burn a lot of API tokens and get nothing much out of it.
...
*** The project
The project is a small book trading web application for the Buenos Aires local area[fn:1]:
1. Users publish a list of books they offer for exchanging, and get to browse other user's offered books.
   [Screenshot]
2. When a users sees a book they like, they request to an exchange to the owner.
   [Screenshot]
3. The book owner receives an email with the exchange request.
   [Screenshot]
4. If the request book owner is interested in any of the other user's book, they get in touch a their convenience and trade books. Most of this happens outside of the application, e.g. via email. (There are no incentives to keep users inside the app; email or WhatsApp work much better for coordinating an exchange).

*** Goals
- Finishing the project, with the exact UX I was going for (which was very simple anyway)
- Minimizing my effort (including frustration and disgust with Claude)
...
*** Stack
I decided to use Django for this project, with some vanilla JavaScript for a few client-side features. Django was my bread and butter until I moved out of Python and web development about 10 years ago. I forgot most of its details but I have a good idea of the concepts and the built-in components, and I felt that using it would safely get most of the web boilerplate out of the way.

For the rest of the components I would just replicate what I'd been using recently on my other projects: a Debian VPS running nginx and gunicorn, sqlite for the database, and Bulma for CSS.

*** Claude Code
Even though my previous attempt at coding with agents didn't work out, the combination of goals of non-goals of this project made it a particularly good fit <for trying again>
...
Beyond the particulars of this project, I had a few reasons to try again:
...
- another critical difference is that this time I was working on a greenfield project as opposed to trying to update a preexisting one
- I suppose CC got a lot better in the few months since I first tried

<using agents wasn't a fundamental factor in this project: I first made my mind about doing it, then decided to see how far I could go with CC. Had I had a first few unproductive sessions I would start from scratch writing all the code myself. (perhaps running a higher risk of dropping the project before completion)
...
[fn:1] This is not an original idea: there's already a similar application that worked pretty well for a few years, until they introduced an aggressive subscription model. This change stagnated the community---both for free and paid users---, so the idea of my project was to provide a free replacement.
