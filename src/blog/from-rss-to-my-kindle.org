...
My first instinct was to try to get away with a Kindle integration that didn't require sending emails from my Flask app. I found a [[https://github.com/maxdjohnson/stkclient][Python library]] that "impersonated" an Amazon  client and wrote my first implementation around it, but it turned out to be brittle: it required storing device credentials in the reader app and manually authenticating every few days, which ultimately made me stop using the feature.

So I looked into the more reasonable option of packaging articles into EPUB files and sending them to my Kindle email. This is what the process looked like from the web app endpoint:
...
Get the cleaned up HTML from an article, package into an EPUB file, send it as the attachment to an email to the Kindle device. Let's look at each of these steps in turn.
...
I found that some websites rely on JavaScript to load images lazily, so I tweaked the tags to force them to render:
