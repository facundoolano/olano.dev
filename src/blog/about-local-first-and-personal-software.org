...
In this scenario, client applications can operate locally without coordinating user operations with servers.
The cloud components are only used for synchronizing data between users and devices, allowed to access only the parts that the users allow them too. The user would pay the provider for the cloud storage, but the communication between the sync service and the local-first applications would be through interoperable protocols, such that the user would be free to switch providers, e.g. migrating from AWS to Azure, or opting for a P2P network, their own self-hosted server, or unplugging altogether and using the app in local-only mode.

Similar to the vision of the [[https://knightcolumbia.org/content/protocols-not-platforms-a-technological-approach-to-free-speech][Protocols, not Platforms]] essay, in this setup cloud providers still have a business, only one not based on locking the users in. The fact that servers are interoperable and application-agnostic means that multiple solutions for the same apps could flourish, catering to different users preferences. This would enable  free and open source multi-device applications without a server administration overhead.
